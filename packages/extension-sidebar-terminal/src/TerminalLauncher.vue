  ...
  <div class="content" @click="handleContentClick">
    ...
    <div :class="{ 'actions-area': true, editing: manageMode }">
      <div ref="actionsRef" :class="{ actions: true, editing: manageMode }">
        <button class="btn primary" @click.stop="emit('primary-click')">
        ...
        >
          <button
            ...
            @click.stop="emit('command-click', command)"
          ...
          <button
            ...
            @click.stop="emit('remove-command', command.id)"
          >
...
import { i18n, type CustomCommand } from './lib'
...
const emit = defineEmits<{
  (e: 'add-command'): void
  (e: 'command-click', command: CustomCommand): void
  (e: 'edit-command', command: CustomCommand): void
  (e: 'leave-manage'): void
  (e: 'primary-click'): void
  (e: 'remove-command', id: string): void
  (e: 'reorder-command', oldIndex: number, newIndex: number): void
  ...
  if (!commandSortable) {
    commandSortable = Sortable.create(actionsRef.value, {
      ...
      onEnd: event => {
        ...
        emit('reorder-command', oldIndex, newIndex)
      },
    })
  }

  commandSortable.option('disabled', !props.manageMode)
}
...
watch(() => props.manageMode, () => {
  nextTick(setupCommandSortable)
})

watch(() => props.customCommands, () => {
  nextTick(setupCommandSortable)
}, { deep: true })
</script>
